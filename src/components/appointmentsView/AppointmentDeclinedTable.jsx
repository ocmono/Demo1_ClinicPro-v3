...
const AppointmentDeclinedTable = () => {
  ...
  console.log('Completed Appointments Auth Debug:', {
    user,
    role,
    isDoctor,
    isAdmin,
  ...
  const { whatsappTemplate, followupTemplate, rejectedWhatsappTemplate } = useMessages();
  ...
  const whatsappReminder = (id) => {
    ...
    const message = rejectedWhatsappTemplate.message
  ...
  // Filter completed appointments based on user role
  const getFilteredDeclinedAppointments = useMemo(() => {
    ...
    if (isDoctor && currentDoctorId) {
      filteredAppointments = filteredAppointments.filter(appt => {
        ...
        console.log('Rejected Appointments Doctor filter check:', {
  ...
  const tableData = useMemo(() => {
    return getFilteredDeclinedAppointments
      .map((appt, index) => {
  ...
  }, [getFilteredDeclinedAppointments]);
  ...
  return (
    <>
      <div>
        <Table data={tableData} columns={columns} emptyMessage={"No Rejected Appointments found"} defaultSorting={[{ id: 'sortableDateTime', desc: true }]}
          cardHeader={
            <h5 className="card-title">Rejected Appointments</h5>
            } />
      </div>
    </>
  );
...
export default AppointmentDeclinedTable;